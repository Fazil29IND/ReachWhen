...
MODEL_PATH = os.path.join(os.path.dirname(__file__), 'Machine Learning Model.pkl')
...
def home():
    ...
    <html>
    <head>
        <title>ReachWhen - Train Journey Duration Predictor</title>
        <style>
            body {
                    ...
                    if (data.error) {
                        ...
                        document.getElementById('result').innerHTML = `
                            <div style="border: 1px solid #ddd; padding: 20px; border-radius: 5px; background: #f9f9f9;">
                                ...
                                <p><strong>Train:</strong> ${data.train_no} (${data.from_station} → ${data.to_station})</p>
...
def predict():
    try:
        ...
        arr_time = request_data.get('arr_time')

        if not all([train_no, from_station, to_station, str(distance), str(stops), dept_time, arr_time]):
            return jsonify({'error': 'Missing required fields'}), 400

        dept_mins = time_to_minutes(dept_time)
        arr_mins = time_to_minutes(arr_time)

        if arr_mins < dept_mins:
            arr_mins += 24 * 60

        actual_duration = arr_mins - dept_mins
        ...
        try:
            ...
            sql = """
                ...
                (TrainNo, PickUpStation, DropUpStation, Distance, NumberOfStops, 
            ...
            values = (
                ...
                dept_time, arr_time, actual_duration, int(predicted_duration)
            )
